...
int main(){
	int a, b, n, m, c, k, u, v, p;
	while(cin>>n>>m>>c>>k && n != 0){
		g.resize(n+1);
		for(int i = 0; i < m; i++){
			cin >> a >> b >> p;
			if((a == b-1 && b <= c-1) || (b == a-1 && a <= c-1) || (a > c-1 && b > c-1)){
		    	g[a].push_back(Vertice(b,p));
				g[b].push_back(Vertice(a,p));
			}
			if(b > c-1 && a <= c-1) g[b].push_back(Vertice(a,p));
			if(a > c-1 && b <= c-1) g[a].push_back(Vertice(b,p));
		}
		dijkstra(k);
		cout << d[c-1] << endl;
		limpar();
